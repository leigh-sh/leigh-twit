from flask import render_template, flash, redirect, g, session, request, current_user
...
@twit.route('/login', methods=['GET', 'POST'])
def login():
...
@twit.errorhandler(404)
def internal_error(error):
	'''Error handler'''
	return render_template('404.html'), 404

@twit.errorhandler(500)
def internal_error(error):
	'''Error handler'''
	db.session.rollback()
